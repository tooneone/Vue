...
export const utils = {
  ...
  isMobile: function () {
    ...
    if ((/AppleWebKit.*mobile/i.test(u)) || (this._isMobile()) || (/android/i.test(u)) || (/MIDP|SymbianOS|NOKIA|SAMSUNG|LG|NEC|TCL|Alcatel|BIRD|DBTEL|Dopod|PHILIPS|HAIER|LENOVO|MOT-|Nokia|SonyEricsson|SIE-|Amoi|ZTE/.test(u))) {
      return true
    } else {
      return false
    }
  ...
  isEmptyObject: function (item) {
    ...
    if (!this.is('Object', item)) { console.log('不是对象'); return }
    let empty = true
    for (let a in item) {
      if (item[a]) empty = false
      break
    }
    return empty
  },
